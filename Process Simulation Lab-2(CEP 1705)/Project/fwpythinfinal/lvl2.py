...
rules= label(pos=(0,0,0),text="""THIS IS LEVEL 2 NOW
THINGS ARE GETTING TENSE!
YOU HAVE 10 BALLS TO FIRE
...
class Earth:
    ...
    def __init__(self):
        ...
        self.planet2=sphere(pos=(500,0,0),radius=self.radius2, material=materials.earth)
        self.atm2=sphere(pos=(500,0,0),radius=10*self.radius2, color=color.blue, opacity=0.1)
    ...
    def gravity(self, o, dt):
        if self.is_in_range(o):
            if o.pos[0]<self.t:   
                ...
                o.v+=a*dt
    def rotate(self, dt):
        self.planet2.rotate(angle=self.rotation2*dt, axis=(0,1,0)) #rotation for asthetics
...
class Balls:
    ...
    def add_ball(self,at_pos, with_speed):
        ...
        ball_c.text='balls left-> %s'%(str(10-self.p))
        
#((((b.pos[0]-cyl.pos[0])**2)/(0.1*cyl.radius**2))+(((b.pos[1]-cyl.pos[1])**2)/(0.98*cyl.radius**2)))<=1
    def move(self,dt):
        for b in self.balls:    
            if self.p<=10:                        
                ...
                if self.planet.hit(b):
                    self.balls.remove(b)
                h2.color=color.yellow
                if ((((b.pos[0]-h2.pos[0])**2)/(0.1*h2.radius**2))+(((b.pos[1]-h2.pos[1])**2)/(0.98*h2.radius**2)))<=1:
                    h2.color=color.blue
...
def main_loop():
    while true:
        ...
        balls.move(deltat)
        earth.rotate(deltat)
        if cyl.color==color.blue and h2.color==color.blue:
            win.text='YOU WON!!!!'
            break
...
deltat=0.01
cyl=ring(pos=(14000,3000,0),radius=1200,color=color.yellow) #the hoop
hoop=label(pos=(14000,-700,0),text='HOOP')
h2=ring(pos=(-10500,-1500,0),radius=1000,color=color.yellow)
main_loop()
